...
def update(data):
    if 'azimuth_static' in data:
        expected_len = range_bins * tx_azimuth_antennas * rx_antennas * 2
        actual_len = len(data['azimuth_static'])
        if expected_len != actual_len:
            print(f"Azimuth status error: expected length {expected_len}, actual {actual_len}", file=sys.stderr, flush=True)
    ...
    zi = spi.griddata((x.ravel(), y.ravel()), a.ravel(), (xi, yi), method='nearest')
    zi = zi[:-1,:-1]
...
if __name__ == "__main__":
    ...
    try:
        ...
        range_width, grid_res = range_depth / 2, 100  # 减少网格分辨率以提高性能
