...
_meta_ = {
    ...
    'ver': ('03.04.00.03', '03.05.00.04', '03.06.00.00', '03.06'),
    'cli': 'mmwDemo:/>',
...
def aux_buffer(input, output, head=40, indices={
    ...
    def aux_point(dat, n=12):  # SDK 3.x point format (12 bytes)
        # 0-1: rangeIdx, 2-3: dopplerIdx, 4-5: peakVal, 6-7: x, 8-9: y, 10-11: z
        p = intify(dat[4:6])
        x = intify(dat[6:8])
        y = intify(dat[8:10])
        z = intify(dat[10:12])
    ...
    # 7) point cloud side info - 4 bytes per point (SNR + noise)
    if address == 7 and values > 0:
        # Consume all side_info data at once
    ...
    # 1) point cloud - 16-byte per point format (x, y, z, v as float32)
    if address == 1 and values > 0:
        num_points = output.get('header', {}).get('objects', 0)
        
        idx = 0
        while len(buffer) >= 16 and values >= 8:
            # 16-byte point: x(4), y(4), z(4), v(4) as IEEE 754 float32
            x = struct.unpack('<f', buffer[0:4])[0]
            y = struct.unpack('<f', buffer[4:8])[0]
            z = struct.unpack('<f', buffer[8:12])[0]
            p = struct.unpack('<f', buffer[12:16])[0]
            
            buffer = buffer[16:]
            values -= 8  # 16 bytes = 8 words
            try:
                output[indices[address]]['{},{}'.format(idx, idx)] = {'v': p, 'x': x, 'y': y, 'z': z}
            except:
                pass
            idx += 1
        
        # Always reset address when done or when values exhausted
        if values < 8:
